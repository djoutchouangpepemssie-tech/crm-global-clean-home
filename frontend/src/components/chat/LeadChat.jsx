    ...
    <div className="flex flex-col" style={{height: "400px"}}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-semibold text-neutral-200">Chat avec {leadName}</p>
        </div>
        <button onClick={load} className="p-1.5 rounded-lg hover:bg-neutral-50 text-neutral-500 hover:text-neutral-300">
          <RefreshCw className="w-3.5 h-3.5" />
        </button>
      </div>

      <div className="flex-1 overflow-y-auto space-y-3 p-3 bg-neutral-100 rounded-xl border border-neutral-100 mb-3">
        {loading ? (
          ...
          messages.map((msg, i) => (
            <div key={i} className={"flex " + (msg.from_client ? "justify-start" : "justify-end")}>
              <div className={"max-w-xs " + (msg.from_client ? "" : "")}>
                <p className={"text-[10px] font-semibold mb-1 " + (msg.from_client ? "text-neutral-500" : "text-brand-400 text-right")}>
                ...
                <div className={"px-4 py-2.5 rounded-2xl text-sm " + (msg.from_client
                  ? "bg-white border border-neutral-200 text-neutral-200 rounded-tl-sm"
                  : "bg-brand-600 text-white rounded-tr-sm")}>
                  {msg.content}
                </div>
                <p className={"text-[10px] text-neutral-600 mt-1 " + (msg.from_client ? "" : "text-right")}>
      ...
      <div className="flex gap-2">
        <input value={newMessage} onChange={e => setNewMessage(e.target.value)}
          ...
          className="flex-1 px-3 py-2.5 bg-white border border-neutral-200 text-neutral-200 rounded-xl text-sm focus:outline-none focus:border-brand-500" />
        <button onClick={handleSend} disabled={sending || !newMessage.trim()}
          className="px-4 py-2.5 bg-brand-600 hover:bg-brand-500 text-white font-bold rounded-xl disabled:opacity-50 transition-all">
