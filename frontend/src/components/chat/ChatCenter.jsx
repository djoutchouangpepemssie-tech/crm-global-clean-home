      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{height: "calc(100vh - 200px)"}}>
        ...
        <div className="bg-white border border-neutral-200 rounded-xl overflow-hidden flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-neutral-100">
            <p className="text-sm font-semibold text-neutral-200">{conversations.length} conversations</p>
            <button onClick={load} className="p-1.5 rounded-lg hover:bg-neutral-50 text-neutral-500">
              ...
              conversations.map((conv, i) => {
                ...
                return (
                  <div key={i} onClick={() => setSelected(conv)}
                    className={"flex items-center gap-3 p-4 cursor-pointer transition-all border-b border-neutral-200 " +
                      (isSelected ? "bg-brand-500/10 border-l-2 border-l-brand-500" : "hover:bg-white")}>
                    <div className="w-10 h-10 rounded-full bg-brand-500/20 border border-brand-500/30 flex items-center justify-center text-brand-300 font-bold text-sm flex-shrink-0">
                      {(conv.lead_name || "?").charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold text-neutral-200 truncate">{conv.lead_name}</p>
                        {conv.unread_crm > 0 && (
            ...
            <>
              <div className="flex items-center justify-between mb-4 pb-4 border-b border-neutral-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-neutral-200">{selected.lead_name}</p>
                    <p className="text-xs text-neutral-500">{selected.lead_email}</p>
                  </div>
                </div>
