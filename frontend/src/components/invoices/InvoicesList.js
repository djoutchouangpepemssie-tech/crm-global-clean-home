...
const InvoicesList = () => {
  ...
  return (
    <div className="p-8" data-testid="invoices-page">
      ...
      {/* Filters */}
      <div className="flex gap-2 mb-6" data-testid="invoice-filters">
        {filters.map(f => (
          <button
            key={f.value}
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden" data-testid="invoices-table">
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-slate-400 mt-2">Créez une facture depuis un devis accepté</p>
          </div>
        ) : (
          <table className="w-full">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Facture</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Montant TTC</th>
                ...
                return (
                  <tr key={inv.invoice_id} data-testid={`invoice-row-${inv.invoice_id}`} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      <span className="font-mono text-sm text-slate-900">{inv.invoice_id}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-medium text-slate-900">{inv.lead_name}</div>
                      <div className="text-sm text-slate-500">{inv.lead_email}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-900">{inv.service_type}</td>
                    <td className="px-6 py-4 font-semibold text-slate-900">{formatCurrency(inv.amount_ttc)}</td>
                    <td className="px-6 py-4">
