...
const COLORS = ["#047857","#60a5fa","#047857","#f59e0b","#c2410c","#06b6d4"];
  ...
  return (
    <div className="bg-white border border-neutral-200 rounded-xl p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        {change && (
          <span className={"flex items-center gap-0.5 text-[10px] font-bold px-2 py-0.5 rounded-full " + (trend==="up"?"bg-brand-500/10 text-brand-400 border border-brand-500/20":"bg-terracotta-500/10 text-terracotta-400 border border-terracotta-500/20")}>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <MetricCard title="CA total" value={totalCA.toLocaleString()+"EUR"} subtitle="Ce mois" icon={DollarSign} color="#047857" trend="up" change="+12%" />
        <MetricCard title="Interventions" value={totalInterventions} subtitle="Realisees" icon={Zap} color="#60a5fa" trend="up" change="+8%" />
        <MetricCard title="Marge nette moy." value={avgMarge+"%"} subtitle="Tous services" icon={Target} color="#d97706" trend="up" change="+3%" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-neutral-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-neutral-200 mb-4">CA par service</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={SERVICES_DATA}>
              <CartesianGrid strokeDasharray="3 3" stroke="var(--border-default, rgba(0,0,0,0.06))" />
              <XAxis dataKey="service" stroke="#78716c" style={{fontSize:"9px"}} tickLine={false} axisLine={false}
                tickFormatter={v => v.split(" ")[0]} />
              <YAxis stroke="#78716c" style={{fontSize:"10px"}} tickLine={false} axisLine={false} width={40} />
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-neutral-200 mb-4">Marge nette par service</h3>
          <div className="space-y-3">
            {SERVICES_DATA.sort((a,b)=>b.marge_nette-a.marge_nette).map((s,i) => (
              <div key={i}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold text-neutral-300">{s.service}</span>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-neutral-500">{s.interventions} missions</span>
                    <span className="text-sm font-black" style={{color: s.marge_nette>=60?"#047857":s.marge_nette>=50?"#f59e0b":"#c2410c"}}>
                      {s.marge_nette}%
                    </span>
                  </div>
                </div>
                <div className="h-2 bg-white rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-700"
                    style={{width:s.marge_nette+"%", background: s.marge_nette>=60?"#047857":s.marge_nette>=50?"#f59e0b":"#c2410c"}} />
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-sm font-semibold text-neutral-200">Alertes rentabilite IA</h3>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
          {[
            { emoji:"🏆", color:"#047857", title:"Bureaux = service le plus rentable", detail:"74% de marge brute. Augmentez votre capacite sur ce segment." },
            ...
            <div key={i} className="flex items-start gap-3 p-4 rounded-xl" style={{background:a.color+"08",border:"1px solid "+a.color+"20"}}>
              ...
              <div>
                <p className="text-sm font-semibold text-neutral-200 mb-1">{a.title}</p>
      ...
      {SERVICES_DATA.map((s,i) => (
        <div key={i} className="bg-white border border-neutral-200 rounded-xl p-5">
          <div className="flex items-start justify-between mb-4">
            <div>
              <h3 className="font-bold text-neutral-100">{s.service}</h3>
              <p className="text-xs text-neutral-500 mt-0.5">{s.interventions} interventions · Prix moy. {s.prix_moy}EUR</p>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black" style={{color:s.marge_nette>=60?"#047857":s.marge_nette>=50?"#f59e0b":"#c2410c"}}>
                {s.marge_nette}%
              </p>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              {l:"CA total",v:s.ca.toLocaleString()+"EUR",c:"#047857"},
              {l:"Duree moy.",v:s.duree_moy+"h",c:"#60a5fa"},
              {l:"Cout horaire",v:s.cout_horaire+"EUR",c:"#f59e0b"},
              {l:"Cout produits",v:s.cout_produits+"EUR",c:"#d97706"},
              {l:"Satisfaction",v:"⭐ "+s.satisfaction,c:"#f59e0b"},
            ].map((m,j) => (
              <div key={j} className="p-3 rounded-xl bg-neutral-100 border border-neutral-100 text-center">
                <p className="text-sm font-black" style={{color:m.c}}>{m.v}</p>
                <p className="text-[10px] text-neutral-600 mt-0.5">{m.l}</p>
              </div>
            ))}
          </div>
          <div className="mt-4">
            <div className="flex justify-between mb-1">
              <span className="text-[10px] text-neutral-600">Marge brute: {s.marge_brute}%</span>
              <span className="text-[10px] text-neutral-600">Marge nette: {s.marge_nette}%</span>
            </div>
            <div className="h-2 bg-white rounded-full overflow-hidden">
              <div className="h-full rounded-full" style={{width:s.marge_brute+"%",background:"rgba(4,120,87,0.3)"}} />
            </div>
            <div className="h-2 bg-white rounded-full overflow-hidden mt-1">
              <div className="h-full rounded-full" style={{width:s.marge_nette+"%",background:"#047857"}} />
            </div>
          </div>
        </div>
      ))}
...
function ZonesTab() {
  return (
    <div className="space-y-4">
      <div className="bg-white border border-neutral-200 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-neutral-200 mb-4">Rentabilite par zone geographique</h3>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-neutral-100">
                {["Zone","Leads","CA","Cout deplac.","Marge nette","Top service"].map(h => (
                  <th key={h} className="text-left text-xs font-semibold text-neutral-500 py-3 px-3">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {ZONES_DATA.sort((a,b)=>b.marge_nette-a.marge_nette).map((z,i) => (
                <tr key={i} className="border-b border-neutral-200 hover:bg-neutral-100 transition-colors">
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5 text-neutral-500" />
                      <span className="text-sm font-semibold text-neutral-200">{z.zone}</span>
                    </div>
                  </td>
                  <td className="py-3 px-3 text-sm text-neutral-400">{z.leads}</td>
                  <td className="py-3 px-3 text-sm font-bold text-neutral-200">{z.ca.toLocaleString()}EUR</td>
                  <td className="py-3 px-3">
                    <span className={"text-xs font-semibold px-2 py-0.5 rounded-full " + (z.cout_deplacement>20?"bg-terracotta-500/10 text-terracotta-400":"bg-brand-500/10 text-brand-400")}>
                      {z.cout_deplacement}EUR
                    </span>
                  </td>
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 bg-neutral-50 rounded-full overflow-hidden">
                        <div className="h-full rounded-full" style={{width:z.marge_nette+"%",background:z.marge_nette>=60?"#047857":z.marge_nette>=50?"#f59e0b":"#c2410c"}} />
                      </div>
                      <span className="text-xs font-black" style={{color:z.marge_nette>=60?"#047857":z.marge_nette>=50?"#f59e0b":"#c2410c"}}>
                        {z.marge_nette}%
                      </span>
                    </div>
                  </td>
                  <td className="py-3 px-3">
                    <span className="text-xs px-2 py-0.5 rounded-full bg-brand-500/10 text-brand-300 border border-brand-500/20">{z.top_service}</span>
                  </td>
                </tr>
              ))}
            </tbody>
...
function CanauxTab() {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white border border-neutral-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-neutral-200 mb-4">CA par canal</h3>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={CANAUX_DATA} layout="vertical">
              ...
              <XAxis type="number" stroke="#78716c" style={{fontSize:"10px"}} tickLine={false} axisLine={false} />
              <YAxis dataKey="canal" type="category" stroke="#78716c" style={{fontSize:"10px"}} tickLine={false} axisLine={false} width={100} />
              <Tooltip contentStyle={{background:"var(--bg-card)",border:"1px solid var(--border-default)",borderRadius:"8px",fontSize:"12px"}}
                formatter={v => [v+"EUR","CA"]} />
        ...
        <div className="bg-white border border-neutral-200 rounded-xl p-5">
          <h3 className="text-sm font-semibold text-neutral-200 mb-4">ROAS et CPA par canal</h3>
          <div className="space-y-3">
            {CANAUX_DATA.map((c,i) => (
              <div key={i} className="p-3 rounded-xl bg-neutral-100 border border-neutral-100">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-bold text-neutral-200">{c.canal}</span>
                  <div className="flex gap-3">
                    <span className="text-[10px] text-neutral-500">CPA: <span className="font-bold text-neutral-300">{c.cpa > 0 ? c.cpa+"EUR" : "Gratuit"}</span></span>
                    <span className="text-[10px] text-neutral-500">ROAS: <span className="font-bold" style={{color:c.roas>=5?"#047857":c.roas>=3?"#f59e0b":c.roas?"#c2410c":"#047857"}}>{c.roas ? c.roas+"x" : "∞"}</span></span>
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="text-center">
                    <p className="text-xs font-black text-neutral-200">{c.leads}</p>
                    <p className="text-[9px] text-neutral-600">Leads</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs font-black text-neutral-200">{c.ca.toLocaleString()}EUR</p>
                    <p className="text-[9px] text-neutral-600">CA</p>
                  </div>
                  <div className="text-center">
                    <p className="text-xs font-black" style={{color:c.marge_nette>=60?"#047857":c.marge_nette>=50?"#f59e0b":"#c2410c"}}>{c.marge_nette}%</p>
                    <p className="text-[9px] text-neutral-600">Marge</p>
                  </div>
                </div>
              </div>
            ))}
    ...
    <div className="space-y-5">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        <div className="bg-white border border-neutral-200 rounded-xl p-5 text-center">
          ...
          <p className="text-3xl font-black text-neutral-100">{prevMois.toLocaleString()}EUR</p>
          <p className="text-xs text-neutral-500 mt-1">Prevision Novembre</p>
          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-brand-500/10 text-brand-400 border border-brand-500/20 mt-2">
            <ArrowUpRight className="w-3 h-3" />+{croissance}% vs Oct
          </span>
        </div>
        <div className="bg-white border border-neutral-200 rounded-xl p-5 text-center">
          <TrendingUp className="w-8 h-8 text-neutral-400 mx-auto mb-2" />
          <p className="text-3xl font-black text-neutral-100">{prevTrimestre.toLocaleString()}EUR</p>
          <p className="text-xs text-neutral-500 mt-1">Prevision Q4 2025</p>
          <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-neutral-500/10 text-neutral-400 border border-neutral-500/20 mt-2">
            Confiance: 87%
          </span>
        </div>
        <div className="bg-white border border-neutral-200 rounded-xl p-5 text-center">
          <Target className="w-8 h-8 text-amber-400 mx-auto mb-2" />
          <p className="text-3xl font-black text-neutral-100">245k EUR</p>
          <p className="text-xs text-neutral-500 mt-1">Prevision annuelle 2025</p>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5">
        <h3 className="text-sm font-semibold text-neutral-200 mb-4">Evolution CA reel vs previsions IA</h3>
        <ResponsiveContainer width="100%" height={250}>
          <LineChart data={PREVISIONS_DATA}>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--border-default, rgba(0,0,0,0.06))" />
            <XAxis dataKey="mois" stroke="#78716c" style={{fontSize:"10px"}} tickLine={false} axisLine={false} />
            <YAxis stroke="#78716c" style={{fontSize:"10px"}} tickLine={false} axisLine={false} width={50} />
            <Tooltip contentStyle={{background:"var(--bg-card)",border:"1px solid var(--border-default)",borderRadius:"8px",fontSize:"12px"}}
              formatter={(v,n) => [v ? v.toLocaleString()+"EUR" : "-", n==="reel"?"CA reel":"Prevision IA"]} />
            <Line type="monotone" dataKey="reel" stroke="#047857" strokeWidth={2} dot={{fill:"#047857",r:4}} connectNulls={false} name="reel" />
            <Line type="monotone" dataKey="prevision" stroke="#047857" strokeWidth={2} strokeDasharray="5 5" dot={{fill:"#047857",r:4}} connectNulls={false} name="prevision" />
          </LineChart>
        </ResponsiveContainer>
        <div className="flex items-center gap-6 mt-3 justify-center">
          <div className="flex items-center gap-2"><div className="w-4 h-0.5 bg-brand-400" /><span className="text-xs text-neutral-500">CA reel</span></div>
          <div className="flex items-center gap-2"><div className="w-4 h-0.5 bg-brand-400 border-dashed border-t border-brand-400" /><span className="text-xs text-neutral-500">Prevision IA</span></div>
        </div>
      </div>
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <Sparkles className="w-4 h-4 text-brand-400" />
          <h3 className="text-sm font-semibold text-neutral-200">Recommandations IA pour maximiser la rentabilite</h3>
        </div>
        <div className="space-y-3">
          {[
            { emoji:"🎯", color:"#047857", title:"Doubler les interventions bureaux", detail:"Marge de 74% vs 48% pour les tapis. 1 client bureau = 3x la valeur d un client particulier. Creez une offre B2B dediee." },
            ...
            { emoji:"🔄", color:"#d97706", title:"Lancer des contrats d abonnement menage", detail:"Un client menage hebdomadaire = 600EUR/mois garanti vs 150EUR ponctuel. Objectif: 20 abonnes = +12k EUR CA recurrent." },
          ].map((r,i) => (
            <div key={i} className="flex items-start gap-3 p-4 rounded-xl" style={{background:r.color+"08",border:"1px solid "+r.color+"20"}}>
              ...
              <div>
                <p className="text-sm font-bold text-neutral-200 mb-1">{r.title}</p>
        ...
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={"flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap " +
              (activeTab===tab.id?"bg-brand-600 text-white":"text-neutral-500 hover:text-neutral-300 hover:bg-white")}>
