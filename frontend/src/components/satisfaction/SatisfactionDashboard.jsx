...
const NPSGauge = ({ score }) => {
  ...
  const npsColor = score < 0 ? '#f43f5e' : score < 50 ? '#f59e0b' : '#34d399';
  ...
  return (
    <svg viewBox="0 0 200 120" style={{ width: '100%', maxWidth: 260 }}>
      ...
      <path d="M 155 35 A 80 80 0 0 1 180 100" fill="none" stroke="rgba(52,211,153,0.3)" strokeWidth="16" strokeLinecap="round" />
      {/* Needle */}
      <line x1={cx} y1={cy} x2={nx} y2={ny} stroke={npsColor} strokeWidth="3" strokeLinecap="round" />
      <circle cx={cx} cy={cy} r="6" fill={npsColor} />
      {/* Score labels */}
      <text x="16" y="115" fontSize="9" fill="#64748b">-100</text>
      <text x="90" y="17" fontSize="9" fill="#64748b">0</text>
      <text x="174" y="115" fontSize="9" fill="#64748b">100</text>
    </svg>
  );
};
...
const SatisfactionDashboard = () => {
  ...
  const npsColor = nps < 0 ? '#f43f5e' : nps < 50 ? '#f59e0b' : '#34d399';
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 300 }}>
        <RefreshCw style={{ width: 30, height: 30, color: '#8b5cf6', animation: 'spin 1s linear infinite' }} />
    ...
    <div className="crm-p-mobile" style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div className="crm-page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: '#64748b', fontSize: 13, marginTop: 4 }}>NPS, CSAT et avis Google</p>
        </div>
        <button
      ...
      <div className="crm-grid" style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div className="section-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '28px 20px' }}>
          ...
          <NPSGauge score={nps} />
          <p style={{ fontSize: 52, fontWeight: 900, color: npsColor, fontFamily: 'Manrope,sans-serif', lineHeight: 1, marginTop: -10 }}>
            {nps > 0 ? '+' : ''}{nps}
          </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 14 }}>
          {[
            { label: 'Promoteurs', value: `${promoters}%`, icon: ThumbsUp, color: '#34d399', bg: 'rgba(52,211,153,0.1)' },
            { label: 'Passifs', value: `${passives}%`, icon: Minus, color: '#f59e0b', bg: 'rgba(245,158,11,0.1)' },
            { label: 'Détracteurs', value: `${detractors}%`, icon: ThumbsDown, color: '#f43f5e', bg: 'rgba(244,63,94,0.1)' },
            { label: 'Total enquêtes', value: totalSurveys, icon: Users, color: '#8b5cf6', bg: 'rgba(139,92,246,0.1)' },
      ...
      <div className="crm-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
        ...
        <div className="section-card">
          ...
          {trendData.length === 0 ? (
            <div style={{ height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569', fontSize: 13 }}>Pas de données disponibles</div>
          ) : (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={trendData}>
                ...
                <Line type="monotone" dataKey="nps" stroke="#8b5cf6" strokeWidth={2.5} dot={{ fill: '#8b5cf6', r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        {/* Per-intervenant */}
        <div className="section-card">
          <h3 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 14, fontWeight: 800, color: '#f1f5f9', marginBottom: 16 }}>NPS par Intervenant</h3>
          {intervenants.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {intervenants.slice(0, 6).map((inter, i) => {
                ...
                const color = score < 0 ? '#f43f5e' : score < 50 ? '#f59e0b' : '#34d399';
                const pct = ((score + 100) / 200) * 100;
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(139,92,246,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, color: '#a78bfa', flexShrink: 0 }}>
      ...
      <div className="crm-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 20 }}>
        ...
        <div className="section-card">
          ...
          {surveys.length === 0 ? (
            <p style={{ color: '#64748b', fontSize: 13, textAlign: 'center', padding: 30 }}>Aucune enquête disponible</p>
          ) : (
                ...
                <div key={s.id || i} style={{ padding: '12px 14px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
                    <div>
                      ...
                      {s.intervenant_name && <span style={{ fontSize: 11, color: '#64748b', marginLeft: 8 }}>• {s.intervenant_name}</span>}
                    </div>
                    <span style={{ fontSize: 11, color: '#475569' }}>{s.created_at ? new Date(s.created_at).toLocaleDateString('fr-FR') : ''}</span>
                  </div>
                  <StarRating score={s.rating || s.score} />
                  {s.comment && <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 6, lineHeight: 1.5, fontStyle: 'italic' }}>"{s.comment}"</p>}
                </div>
              ))}
            </div>
          )}
        ...
        {/* Google Review stats */}
        <div className="section-card">
          <h3 style={{ fontFamily: 'Manrope,sans-serif', fontSize: 14, fontWeight: 800, color: '#f1f5f9', marginBottom: 16 }}>
            Avis Google
          </h3>
          {googleStats ? (
            <div>
              <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
                <div style={{ flex: 1, padding: 14, background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.15)', borderRadius: 10, textAlign: 'center' }}>
                  <p style={{ fontSize: 24, fontWeight: 800, color: '#34d399', fontFamily: 'Manrope,sans-serif' }}>{googleStats.reviews_left || 0}</p>
                  <p style={{ fontSize: 11, color: '#64748b' }}>Laissés</p>
                </div>
                <div style={{ flex: 1, padding: 14, background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.15)', borderRadius: 10, textAlign: 'center' }}>
                  <p style={{ fontSize: 24, fontWeight: 800, color: '#a78bfa', fontFamily: 'Manrope,sans-serif' }}>{googleStats.reviews_requested || 0}</p>
                  <p style={{ fontSize: 11, color: '#64748b' }}>Demandés</p>
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                  <span style={{ fontSize: 12, color: '#94a3b8' }}>Taux de conversion</span>
              ...
              {googleStats.avg_rating && (
                <div style={{ padding: 14, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.15)', borderRadius: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    ...
                    <div>
                      <p style={{ fontSize: 22, fontWeight: 800, color: '#f59e0b', fontFamily: 'Manrope,sans-serif', margin: 0 }}>{googleStats.avg_rating.toFixed(1)}</p>
                      <p style={{ fontSize: 11, color: '#64748b', margin: 0 }}>Note moyenne Google</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            <p style={{ color: '#64748b', fontSize: 13, textAlign: 'center', padding: 30 }}>Données non disponibles</p>
          )}
        </div>
      </div>
