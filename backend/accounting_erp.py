...
class ExpenseCreate(BaseModel):
    ...
    status: str = "payee"  # payee | en_attente
...
async def create_journal_entry(
    ...
    await _db.erp_journal_entries.insert_one(doc)
    return entry_id
...
async def create_expense_journal_entry(expense: dict, user_id: str = None):
    ...
    credit_account = "512" if expense.get("status") == "payee" else "401"
...
async def get_dashboard_kpis(request: Request):
    ...
    pipeline_ca = [
        {"$match": {"status": {"$in": ["envoyee", "payee"]}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]
    ca_all = await _db.erp_invoices.aggregate(pipeline_ca).to_list(1)
    ca_total = ca_all[0]["total"] if ca_all else 0
    
    # CA jour
    ca_day_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": today_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ca_day = ca_day_agg[0]["total"] if ca_day_agg else 0
    
    # CA mois
    ca_month_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ca_month = ca_month_agg[0]["total"] if ca_month_agg else 0
    
    # CA année
    ca_year_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": year_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ca_year = ca_year_agg[0]["total"] if ca_year_agg else 0
    
    # CA mois dernier (pour variation %)
    ca_last_month_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": last_month_start, "$lt": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ca_last_month = ca_last_month_agg[0]["total"] if ca_last_month_agg else 0
    
    # Dépenses mois
    dep_month_agg = await _db.erp_expenses.aggregate([
    ...
    dep_last_agg = await _db.erp_expenses.aggregate([
    ...
    ca_ht_month_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ht"}}},
    ]).to_list(1)
    ca_ht_month = ca_ht_month_agg[0]["total"] if ca_ht_month_agg else 0
    
    dep_ht_month_agg = await _db.erp_expenses.aggregate([
        {"$match": {"date": {"$gte": month_start}}},
    ...
    # Variation % bénéfice
    ca_ht_last_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": last_month_start, "$lt": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ht"}}},
    ]).to_list(1)
    ca_ht_last = ca_ht_last_agg[0]["total"] if ca_ht_last_agg else 0
    dep_ht_last_agg = await _db.erp_expenses.aggregate([
        {"$match": {"date": {"$gte": last_month_start, "$lt": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$amount_ht"}}},
    ]).to_list(1)
    ...
    config = await _db.erp_treasury_config.find_one({"_id": "config"})
    ...
    entries_in = await _db.erp_invoices.aggregate([
        {"$match": {"status": "payee"}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ...
    entries_out = await _db.erp_expenses.aggregate([
        {"$match": {"status": "payee"}},
        {"$group": {"_id": None, "total": {"$sum": "$amount_ttc"}}},
    ]).to_list(1)
    ...
    pending_invoices = await _db.erp_invoices.aggregate([
        {"$match": {"status": "envoyee"}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    pending_in = pending_invoices[0]["total"] if pending_invoices else 0
    
    pending_expenses = await _db.erp_expenses.aggregate([
    ...
    # Factures impayées
    unpaid = await _db.erp_invoices.count_documents({"status": "envoyee"})
    unpaid_amount_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": "envoyee"}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    ...
    overdue = await _db.erp_invoices.count_documents({"status": "envoyee", "invoice_date": {"$lt": overdue_date}})
    
    # Dépenses en attente
    pending_exp_count = await _db.erp_expenses.count_documents({"status": "en_attente"})
...
async def get_dashboard_charts(request: Request):
    ...
    for i in range(11, -1, -1):
        ...
        agg = await _db.erp_invoices.aggregate([
            {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": m_start, "$lt": m_end}}},
            {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
        ]).to_list(1)
        ca_monthly.append({
    ...
    prestation_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}}},
    ...
    month_start = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0).isoformat()
    ca_ht = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ht"}}},
    ]).to_list(1)
    charges_ht = await _db.erp_expenses.aggregate([
        {"$match": {"date": {"$gte": month_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$amount_ht"}}},
    ]).to_list(1)
    ...
    for i in range(5, -1, -1):
        ...
        in_agg = await _db.erp_invoices.aggregate([
            {"$match": {"status": "payee", "payment_date": {"$gte": m_start, "$lt": m_end}}},
            {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
        ]).to_list(1)
        out_agg = await _db.erp_expenses.aggregate([
            {"$match": {"status": "payee", "date": {"$gte": m_start, "$lt": m_end}}},
            {"$group": {"_id": None, "total": {"$sum": "$amount_ttc"}}},
        ]).to_list(1)
...
async def list_invoices(
    ...
    total = await _db.erp_invoices.count_documents(query)
    skip = (page - 1) * page_size
    items = await _db.erp_invoices.find(query, {"_id": 0}).sort("created_at", -1).skip(skip).limit(page_size).to_list(page_size)
    
    # Auto-flag overdue
    for inv in items:
        if inv.get("status") == "envoyee" and inv.get("invoice_date", "") < overdue_date:
...
async def create_invoice(inp: InvoiceCreate, request: Request):
    ...
    await _db.erp_invoices.insert_one(invoice)
...
async def get_invoice(invoice_id: str, request: Request):
    """Détails facture."""
    await _require_auth(request)
    inv = await _db.erp_invoices.find_one({"invoice_id": invoice_id, "deleted_at": {"$exists": False}}, {"_id": 0})
    if not inv:
        raise HTTPException(404, "Facture introuvable")
    
    # Get related journal entries
    journals = await _db.erp_journal_entries.find({"reference_id": invoice_id}, {"_id": 0}).to_list(20)
...
async def update_invoice(invoice_id: str, inp: InvoiceUpdate, request: Request):
    ...
    inv = await _db.erp_invoices.find_one({"invoice_id": invoice_id, "deleted_at": {"$exists": False}})
    if not inv:
        raise HTTPException(404, "Facture introuvable")
    ...
    await _db.erp_invoices.update_one({"invoice_id": invoice_id}, {"$set": update})
    
    return await _db.erp_invoices.find_one({"invoice_id": invoice_id}, {"_id": 0})
...
async def send_invoice(invoice_id: str, request: Request):
    ...
    # 1. Change status
    await _db.erp_invoices.update_one(
    ...
    return {"status": "envoyee", "journal_entry_id": journal_id, "message": "Facture envoyée, écriture comptable générée"}
...
async def record_payment(invoice_id: str, inp: PaymentRecord, request: Request):
    ...
    if not inv:
        raise HTTPException(404, "Facture introuvable")
    if inv.get("status") not in ("envoyee", "en_retard"):
    ...
    return {"status": "payee", "journal_entry_id": journal_id, "amount": amount, "message": "Paiement enregistré, trésorerie mise à jour"}
...
async def archive_invoice(invoice_id: str, request: Request):
    ...
    user = await _require_auth(request)
    inv = await _db.erp_invoices.find_one({"invoice_id": invoice_id})
    if not inv:
        raise HTTPException(404, "Facture introuvable")
...
async def list_expenses(
    ...
    total = await _db.erp_expenses.count_documents(query)
    skip = (page - 1) * page_size
    items = await _db.erp_expenses.find(query, {"_id": 0}).sort("date", -1).skip(skip).limit(page_size).to_list(page_size)
...
async def create_expense(inp: ExpenseCreate, request: Request):
    ...
    expense = {
        ...
        "status": inp.status,
        "payment_date": now if inp.status == "payee" else None,
    ...
    await _db.erp_expenses.insert_one(expense)
...
async def update_expense(expense_id: str, inp: ExpenseUpdate, request: Request):
    user = await _require_auth(request)
    exp = await _db.erp_expenses.find_one({"expense_id": expense_id, "deleted_at": {"$exists": False}})
    if not exp:
    ...
    update["updated_at"] = datetime.now(timezone.utc).isoformat()
    await _db.erp_expenses.update_one({"expense_id": expense_id}, {"$set": update})
    
    return await _db.erp_expenses.find_one({"expense_id": expense_id}, {"_id": 0})
...
async def pay_expense(expense_id: str, request: Request):
    ...
    if not exp:
        raise HTTPException(404, "Dépense introuvable")
    if exp.get("status") == "payee":
        raise HTTPException(400, "Dépense déjà payée")
    
    now = datetime.now(timezone.utc).isoformat()
    await _db.erp_expenses.update_one(
        {"expense_id": expense_id},
        {"$set": {"status": "payee", "payment_date": now, "updated_at": now}}
    )
    ...
    return {"status": "payee", "journal_entry_id": journal_id}


@erp_router.delete("/erp/expenses/{expense_id}")
async def delete_expense(expense_id: str, request: Request):
    user = await _require_auth(request)
    exp = await _db.erp_expenses.find_one({"expense_id": expense_id})
    if not exp:
        raise HTTPException(404, "Dépense introuvable")
    
    await _db.erp_expenses.update_one({"expense_id": expense_id}, {"$set": {"deleted_at": datetime.now(timezone.utc).isoformat()}})
...
async def get_treasury(request: Request, month: Optional[str] = None):
    ...
    in_before = await _db.erp_invoices.aggregate([
        {"$match": {"status": "payee", "payment_date": {"$lt": m_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    out_before = await _db.erp_expenses.aggregate([
        {"$match": {"status": "payee", "date": {"$lt": m_start}}},
        {"$group": {"_id": None, "total": {"$sum": "$amount_ttc"}}},
    ]).to_list(1)
    ...
    entries_in = await _db.erp_invoices.find(
        {"status": "payee", "payment_date": {"$gte": m_start, "$lt": m_end}},
        {"_id": 0, "invoice_id": 1, "invoice_number": 1, "client_name": 1, "total_ttc": 1, "payment_date": 1}
    ).sort("payment_date", 1).to_list(500)
    
    entries_out = await _db.erp_expenses.find(
        {"status": "payee", "date": {"$gte": m_start, "$lt": m_end}, "deleted_at": {"$exists": False}},
    ...
    # Prévision 30j
    pending_in_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": "envoyee"}},
        {"$group": {"_id": None, "total": {"$sum": "$total_ttc"}}},
    ]).to_list(1)
    pending_out_agg = await _db.erp_expenses.aggregate([
        {"$match": {"status": "en_attente"}},
        {"$group": {"_id": None, "total": {"$sum": "$amount_ttc"}}},
    ]).to_list(1)
...
async def update_treasury_config(inp: TreasuryConfig, request: Request):
    await _require_auth(request)
    await _db.erp_treasury_config.update_one(
        {"_id": "config"},
...
async def get_journal(
    ...
    total = await _db.erp_journal_entries.count_documents(query)
    skip = (page - 1) * page_size
    items = await _db.erp_journal_entries.find(query, {"_id": 0}).sort("entry_date", -1).skip(skip).limit(page_size).to_list(page_size)
...
async def get_journal_entry(entry_id: str, request: Request):
    await _require_auth(request)
    entry = await _db.erp_journal_entries.find_one({"entry_id": entry_id}, {"_id": 0})
    if not entry:
...
async def get_tva_summary(month: str, request: Request):
    ...
    tva_collectee_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": m_start, "$lt": m_end}}},
    ...
    tva_deductible_agg = await _db.erp_expenses.aggregate([
    ...
    # Detail by rate
    tva_by_rate_sell = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": m_start, "$lt": m_end}}},
        {"$unwind": "$items"},
        {"$group": {"_id": "$items.tva_rate", "base_ht": {"$sum": "$items.amount_ht"}, "tva": {"$sum": "$items.amount_tva"}}},
    ]).to_list(10)
    
    tva_by_rate_buy = await _db.erp_expenses.aggregate([
        {"$match": {"date": {"$gte": m_start, "$lt": m_end}, "deleted_at": {"$exists": False}}},
...
async def income_statement(request: Request, month: Optional[str] = None):
    ...
    if month:
        ...
        if m == 12:
            ...
            m_end = datetime(y, m + 1, 1, tzinfo=timezone.utc).isoformat()
        date_filter_inv = {"invoice_date": {"$gte": m_start, "$lt": m_end}}
    ...
    # CA HT
    ca_agg = await _db.erp_invoices.aggregate([
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, **date_filter_inv}},
    ...
    # Charges by category
    charges_agg = await _db.erp_expenses.aggregate([
...
async def top_clients(request: Request, limit: int = Query(default=10, ge=1, le=50)):
    ...
    pipeline = [
        {"$match": {"status": {"$in": ["envoyee", "payee"]}, "deleted_at": {"$exists": False}}},
        {"$group": {
    ...
    clients = await _db.erp_invoices.aggregate(pipeline).to_list(limit)
...
async def services_analysis(request: Request):
    ...
    services = await _db.erp_invoices.aggregate(pipeline).to_list(20)
...
async def period_comparison(request: Request, month1: str = None, month2: str = None):
    ...
    async def get_period_data(month_str):
        ...
        ca = await _db.erp_invoices.aggregate([
            {"$match": {"status": {"$in": ["envoyee", "payee"]}, "invoice_date": {"$gte": m_start, "$lt": m_end}}},
            {"$group": {"_id": None, "ht": {"$sum": "$total_ht"}, "ttc": {"$sum": "$total_ttc"}}},
        ]).to_list(1)
        dep = await _db.erp_expenses.aggregate([
            {"$match": {"date": {"$gte": m_start, "$lt": m_end}, "deleted_at": {"$exists": False}}},
...
async def init_erp_indexes():
    ...
    await _db.erp_invoices.create_index("invoice_id", unique=True)
    await _db.erp_invoices.create_index("invoice_number", unique=True)
    await _db.erp_invoices.create_index("status")
    await _db.erp_invoices.create_index("invoice_date")
    await _db.erp_invoices.create_index("client_id")
    await _db.erp_invoices.create_index("client_name")
    
    await _db.erp_expenses.create_index("expense_id", unique=True)
    await _db.erp_expenses.create_index("category")
    await _db.erp_expenses.create_index("date")
    await _db.erp_expenses.create_index("status")
    
    await _db.erp_journal_entries.create_index("entry_id", unique=True)
    await _db.erp_journal_entries.create_index("journal_type")
    await _db.erp_journal_entries.create_index("entry_date")
    await _db.erp_journal_entries.create_index("reference_id")
