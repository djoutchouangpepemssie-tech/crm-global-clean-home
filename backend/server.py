...
async def _send_telegram_message(text: str) -> dict:
    ...
    """
    # Nettoyage agressif : retire TOUT whitespace (espaces, \n, \r, \t)
    # même internes au token. Le copy-paste BotFather peut wrapper le token
    # sur 2 lignes et insérer un \n au MILIEU (pas seulement à la fin).
    bot_token = "".join((os.environ.get("TELEGRAM_BOT_TOKEN") or "").split())
    chat_id = "".join((os.environ.get("TELEGRAM_CHAT_ID") or "").split())
...
from site_tracking import tracker_router, tracker_legacy_router, audience_router, init_site_tracking_db
app.include_router(tracker_router)
app.include_router(tracker_legacy_router)  # Compat /api/tracker/* pour le site existant
app.include_router(audience_router)        # Alias /api/audience/* (anti-adblocker)
